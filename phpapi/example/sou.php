		...
		<form method="get" action="sou.php">
				<span class="input input--kaede">
					...
					<label class="input__label input__label--kaede" for="input-35">
						<span class="input__label-content input__label-content--kaede">请输入手机号码进行查询<button class="address_sub1">查询</button> </span>
				</span>
		</form>
    </div>
    <?php
    if(isset($_GET['dhc'])){
        $con = mysqli_connect('120.79.71.36','root','nbzx0769','zxkz','3306')or die("<script>alert('您的网络似乎出了点问题');window.location.href='http://www.nbzxapi.com/zxkz/sou.php';</script>");
        mysqli_query($con,'set names UTF8')or die("<script>alert('您的网络似乎出了点问题');window.location.href='http://www.nbzxapi.com/zxkz/sou.php';</script>");
        $sql = "select * from skh where dh=".$_GET['get'];
        $cxjg = mysqli_query($con,$sql)or die("<script>alert('您的网络似乎出了点问题');window.location.href='http://www.nbzxapi.com/zxkz/sou.php';</script>");
        if($cxjg) {
            while ($row = mysqli_fetch_all($cxjg)) {
                echo '收件人='.$row['sjr'].'<br>';
                echo '刻章类型'.$row['kzt'].'<br>';
                echo '刻章材质'.$row['cz'].'<br>';
                if($row['kd']){
                    echo "已发货,快递单号为:".$row['kd'].'<br><br><br>';
                }
                else{
                    echo "未发货,马上发货<br><br><br>";
                }
            }
        }else{
